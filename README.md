...
The homepage catalog can be filtered by one or more categories and by a single publisher. Category selections and the publisher selection can be combined, and the active filter IDs are stored in the page URL so a filtered catalog can be bookmarked or shared.
